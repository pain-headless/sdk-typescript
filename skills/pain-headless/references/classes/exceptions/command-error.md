...
### `CommandError`
*extends `PainHeadlessError`*
```ts
constructor(message: string, errorCause?: string, partialData?: any, credits?: { used: number; total: number }): CommandError
```
**Properties:**
...
- `partialData: any` (optional)
- `errorCause: string` (optional)
- `credits: { used: number; total: number }` (optional)
- `name: string`
- `message: string`
